...
When I first got everything together, all the tests were failing. Turned out there were several issues that needed fixing:

### Timestamp Issues

The code was using Unix timestamps but tests expected `NaiveDateTime` objects:
...
### Genesis Block Problems

Tests expected genesis blocks to have `prev_hash: "ZERO_HASH"`, not `nil`.

### Hash Format Problems

Had mixed uppercase/lowercase hashes, needed to standardize on uppercase.

### Function Pattern Conflicts

Two `hash/1` functions with overlapping patterns:

```elixir
# This never gets called!
def hash(%{} = block) do ... end

def hash(data) do ... end  # Matches everything first
```

Fixed by making the block pattern more specific: `hash(%Block{} = block)`.

### Validation Logic Bug

The validation logic had the parameters backwards:
